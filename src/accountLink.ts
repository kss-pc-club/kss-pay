...
import { auth, firestore, userData } from './firebase'
 ...
 * リンク解除の処理
 * @param user - 現在ログイン中のユーザー情報
 * @param providerId - プロバイダのID
 * @param elem - リンク解除ボタンのDOM
 */
const unlinkFlow = (
  user: User | null,
  providerId: string,
  ...
  if (user) {
    unlink(user, providerId)
      .then(() => {
        ...
        $(elem).next('p.status').removeClass('linked')
      })
      .catch((err) => {
        console.error(err)
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  $('button#google').on('click', function () {
    if ($(this).next('p.status').hasClass('linked')) {
      if (confirm('Googleアカウントを紐づけ解除しますか？')) {
        unlinkFlow(auth.currentUser, 'google.com', this)
      }
    } else {
      if (confirm('Googleアカウントを紐づけしますか？')) {
        linkFlow(auth.currentUser, new GoogleAuthProvider(), this)
      }
    }
  })
  $('button#facebook').on('click', function () {
    if ($(this).next('p.status').hasClass('linked')) {
      if (confirm('Facebookアカウントを紐づけ解除しますか？')) {
        unlinkFlow(auth.currentUser, 'facebook.com', this)
      }
    } else {
      if (confirm('Facebookアカウントを紐づけしますか？')) {
        linkFlow(auth.currentUser, new FacebookAuthProvider(), this)
      }
    }
  })
  $('button#twitter').on('click', function () {
    if ($(this).next('p.status').hasClass('linked')) {
      if (confirm('Twitterアカウントを紐づけ解除しますか？')) {
        unlinkFlow(auth.currentUser, 'twitter.com', this)
      }
    } else {
      if (confirm('Twitterアカウントを紐づけしますか？')) {
        linkFlow(auth.currentUser, new TwitterAuthProvider(), this)
      }
    }
  })
  ...
  $('.dl p.ttl#delete').on('click', () => {
    const form = `
      <form action="#" id="emailLinkForm">
        <select id="reauth" required>
          <option selected value="">再認証に利用するサービスを選択</option>
      <!--    <option value="mail" ${
    ...
    $('.modal .modalContainer form#emailLinkForm').on('submit', (e) => {
      ...
      if (reauth === 'mail' && mail && pass) {
        ...
        if (reauth === 'facebook') {
          reauthenticateWithPopup(auth.currentUser, new FacebookAuthProvider())
            .then(userRemove)
            .catch((err) => {
              console.error(err)
              void toast('再認証に失敗しました')
        ...
        if (reauth === 'twitter') {
          reauthenticateWithPopup(auth.currentUser, new TwitterAuthProvider())
            .then(userRemove)
            .catch((err) => {
              console.error(err)
              void toast('再認証に失敗しました')
